...
class PurchasesReturnController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        foreach ($PurchaseReturn as $Purchase_Return) {
            ...
            $item['payment_status'] = $Purchase_Return['payment_status'];
                    ...
                    if ($product_detail['product_variant_id'] !== null) {
                        ...
                        if ($unit_prod && $product_warehouse) {
                            if ($unit_prod['unitPurchase']->operator == '/') {
                                $product_warehouse->qte -= $product_detail['quantity'] / $unit_prod['unitPurchase']->operator_value;
                            } else {
                                $product_warehouse->qte -= $product_detail['quantity'] * $unit_prod['unitPurchase']->operator_value;
                            }
                            $product_warehouse->save();
    ...
    public function delete_by_selection(Request $request)
    {
        ...
        \DB::transaction(function () use ($request) {
            ...
            foreach ($selectedIds as $PurchaseReturn_id) {
                $PurchaseReturn = PurchaseReturn::findOrFail($PurchaseReturn_id);
    ...
    public function Send_SMS(Request $request)
    {
        ...
        $url = url('/Return_Purchase_PDF/' . $request->id);
