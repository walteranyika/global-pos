...
class ReportController extends BaseController
{
    ...
    public function daily_sales_report(Request $request)
    {
        ...
        $data = $service->getSalesDataForPeriod($request->fromDate, $request->toDate);
        return Excel::download(new DailySalesExport($data), 'Daily_Sales_List.xlsx');
    }
    ...
    public function user_sales_report(Request $request)
    {
        ...
        $heading = str_pad("Item", 36, ' ') . str_pad("Total", 12, ' ', STR_PAD_LEFT);
        $printer->setEmphasis(false);
        ...
        foreach ($users as $user) {
            ...
            foreach ($data as $item) {
                $heading = str_pad($item['title'], 36, ' ') . str_pad($item['total'], 12, ' ', STR_PAD_LEFT);
                $printer->text(strtoupper($heading) . "\n");
            }
            $printer->text(str_repeat(".", 48) . "\n");
        }
        ...
        $names = "User Sales Report for \n $from - $to\n";
        $printer->text($names);

        $printer->feed();
        $printer->cut();
        $printer->close();
        $this->user_sales_json($request);
        return response()->json(['success' => true]);
    }

    public function user_sales_json(Request $request)
    {
        $users = User::where('statut', 1)->get();
        $service = new DailyReportService();
        $all_data = [];
        foreach ($users as $user) {
            $data = $service->getSalesUserReport($request->start_date, $request->end_date, $user->id);
            if (empty($data)) {
                continue;
            }
            $user_data = [];
            $user_data['user'] = Str::title(strtolower($user->firstname));
            $user_data['userId'] = $user->id;
            foreach ($data as $item) {
                $user_data[$item['title']]= $item['total'];
            }
            $keys = array_keys($user_data);
            $required_keys = [ "paid", "unpaid", "Cash", "Mpesa","Credit", "Credit Card", "Complimentary", "Other", "partial", "Total_Held_Sales"];
        ...
        return response()->json(["user_sales"=>$all_data]);
    }
}
