...
class ReportController extends BaseController
{
    ...
    public function daily_sales_report()
    {
        ...
        $columns = array('Product', 'Opening_Stock', 'Closing_Stock', 'Sales', 'Price');
        $callback = function() use($tasks, $columns) {
            ...
            foreach ($tasks as $item) {
                ...
                $total += $item->price;
                fputcsv($file, array($row['Product'], $row['Opening_Stock'], $row['Closing_Stock'], $row['Sales'], $row['Price']));
            }
            fputcsv($file, array('Total Sales', '', '', '', $total));
            fclose($file);
        };
